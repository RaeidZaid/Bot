...
async def handler_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if filters.PHOTO.check_update(update):
        ...
        context.bot.send_message(chat_id=update.effective_chat.id , text=f"يا استاذ {update.message.from_user.first_name} عليك ارسال الصورة المراد ازالة الخلفية منها ")
    photo_file = await context.bot.get_file(file_id)
    await photo_file.download_to_drive(custom_path=f"./int/{photo_name}")
    await context.bot.send_message(chat_id=update.effective_chat.id , text=f"من فضلك انتظر قليلا استاذ {update.message.from_user.first_name} انتظر ١٠ ثواني ")
